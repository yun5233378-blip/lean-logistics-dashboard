...
- Socrata API：https://data.usaid.gov/resource/mm7d-nzmf.json
- 用途：作为真实 shipment-level lead time、运费、重量、运输方式和目的国的公开数据源。
- 当前状态：系统已提供后台导入管线，导入成功后写入 `external_shipments` 表。若上游 API 暂时不可达，已有导入记录会保留，后台任务会记录失败原因。
- 运行时保底样本：`data/scms_delivery_history_sample.json` 保存 5 条公开分析页展示的 SCMS Delivery History 真实样本行，来源为 https://shashank-sundi.github.io/HEALTH-COMMODITY-SHIPMENT-PRICE-PREDICTION/ 。
...
`external_shipments` 保存从 USAID 公开 API 导入的 shipment-level 原始记录摘要：

- `source_record_id`：USAID 原始记录 ID
- `destination_country`：目的国
- `shipment_mode`：运输方式
- `scheduled_delivery_date` / `delivered_to_client_date`：公开交付日期字段
- `lead_time_days`：由交付日期计算出的交付偏差/周期字段
- `freight_cost_usd`、`line_item_value_usd`、`weight_kg`：公开成本与重量字段
- `raw_payload`：保留原始 JSON，便于后续重新建模
